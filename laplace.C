...
int main(int argc, char *argv[]) {
    ...
    while ( dt_global > MAX_TEMP_ERROR && iteration <= max_iterations ) {
        ...
        if (my_PE_num != 0) {
            MPI_Recv(&Temperature[0][1], COLUMNS, MPI_DOUBLE, my_PE_num-1, DOWN, MPI_COMM_WORLD, &status);
        }
...
void initialize(int npes, int my_PE_num){
    ...
    for(i = 0; i <= ROWS+1; i++) {
        ...
        Temperature_last[i][COLUMNS+1] = 100.0/npes*my_PE_num + (100.0/npes/ROWS)*i;
    }
